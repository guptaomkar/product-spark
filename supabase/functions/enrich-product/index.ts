...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[enrich-product] Processing: ${mfr} ${mpn}`);
    console.log(`[enrich-product] Requesting ${attributes.length} attributes in one go.`);

    const username = Deno.env.get("OXYLABS_USERNAME");
    const password = Deno.env.get("OXYLABS_PASSWORD");

    if (!username || !password) {
      return new Response(JSON.stringify({ success: false, error: "Oxylabs credentials missing" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // DYNAMIC QUERY: Injects the entire attribute list
    // We use a "Task-First" prompt which is more effective for large sets of data
    const query = `Act as a precision data extractor. Find the technical specifications for the product: ${mfr} ${mpn}.
Fill every attribute in the list below. If a value is unknown, use "N/A".
Return ONLY a valid JSON object. No conversation. No markdown.

Attributes to find: ${attributes.join(", ")}
...
{"attributes": {${attributes.map((a) => `"${a}": "value"`).join(", ")}}}`;
    ...
    // Parse the full set of attributes from the single response
    const enrichedData = parseAttributeValues(responseText, attributes);
...
function extractResponseText(data: any): string {
  try {
    return data.results?.[0]?.content?.response_text || "";
  } catch (e) {
    return "";
  }
}
...
function parseAttributeValues(responseText: string, attributes: string[]): Record<string, string> {
  ...
  for (const attr of attributes) result[attr] = "";
  ...
  try {
    ...
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]);
      const source = parsed.attributes || parsed;

      for (const attr of attributes) {
        if (source[attr] && source[attr] !== "value" && source[attr] !== "val") {
          result[attr] = String(source[attr]);
        }
      }
    }
  } catch (e) {
    console.warn("[enrich-product] JSON Parse failed, the AI likely returned raw text.");
